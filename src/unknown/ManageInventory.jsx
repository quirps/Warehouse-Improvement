...
// ── Mode tabs (Put Away / Move / Verify / Update) ─────────────────────────────
const MODES = [
  {
    id: "putaway",
    icon: "⬡",
...
function BinSelector({ inv }) {
  const tc = inv.dominantType ? T.types[inv.dominantType] : null;

  const handleKeyDown = (e) => {
    if (e.key === "Enter") inv.resolveBin(inv.binInput, inv.region);
  };
...
// ── Right panel: Scan queue + Put Away action ─────────────────────────────────
function PutAwayPanel({ inv }) {
  // Scan input is ALWAYS enabled — not gated on bin selection
  useEffect(() => {
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      {inv.moveStatus !== "success" && (
        <>
          ...
          {/* No bin prompt — shown but doesn't block scanning */}
          {!inv.selectedBin && inv.scannedItems.length === 0 && (
            ...
            >
              💡 Scan items first, then select a bin in the 3D view or type one
              on the left.
            </div>
          )}
          ...
          {inv.scannedItems.length === 0 && (
            ...
            >
              ...
              <div style={{ fontSize: 11, opacity: 0.6, textAlign: "center" }}>
                Scan a barcode or type a part number above and press Enter
                <br />
                <span
                  style={{
                    color: T.textMuted + "70",
                    marginTop: 6,
                    display: "block",
                  }}
                >
                  Try: NMB7663 · NMB7699 · BKS10042 · DVX44821 · BIG00991
                </span>
              </div>
            </div>
          )}
...
export default function ManageInventory() {
  ...
  const [activeMode, setActiveMode] =
    useState !== undefined
      ...
      : ["putaway", () => {}];
  // Use proper import
  const [activeMode2, setActiveMode2] = [null, null]; // placeholder resolved below

  return <ManageInventoryInner />;
}

// ── Actual component (avoids the inline useState hack above) ──────────────────
import { useState } from "react";

function ManageInventoryInner() {
  const inv = useInventory();
  const [activeMode, setActiveMode] = useState("putaway");

  // When user clicks a bin in the 3D viewer
  const handleViewerSelect = (binInfo) => {
    ...
    inv.setRecentLocs
      ? inv.setRecentLocs((prev) =>
          [binInfo.label, ...prev.filter((r) => r !== binInfo.label)].slice(
            0,
            8,
          ),
        )
      : null;
    // Add to recents via resolveBin without re-checking (already from layout)
          ...
          {activeMode === "putaway" ? (
            ...
            >
              <div style={{ fontSize: 36, opacity: 0.2 }}>
                {{ move: "→", verify: "✓", update: "+" }[activeMode]}
              </div>
              <div style={{ fontSize: 14 }}>
                {
                  { move: "Move", verify: "Verify", update: "Update" }[
                    activeMode
                  ]
                }{" "}
                — coming soon
